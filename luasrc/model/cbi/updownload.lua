...
local dlPath
function List()
	dlPath = http.formvalue("dlfile")
	for i, f in ipairs(fs.glob(dlPath.."/*")) do
		attr = fs.stat(f)
		if attr then
			inits2[i] = {}
			inits2[i].name = fs.basename(f)
...
mt2 = tb2:option(DummyValue, "mtime", translate("Modify time"))
ms2 = tb2:option(DummyValue, "modestr", translate("Mode string"))
...
btnrm2.write = function(self, section)
	local v = luci.fs.unlink(dlPath .. "/" .. luci.fs.basename(inits2[section].name))
	if v then table.remove(inits2, section) end
	return v
end
